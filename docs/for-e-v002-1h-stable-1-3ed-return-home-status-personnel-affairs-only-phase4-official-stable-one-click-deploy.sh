...
(
  ...
  PACKAGE="for-e-v002-1h-stable-1-3ed-return-home-status-personnel-affairs-only-phase4-official-stable"
  VERSION="V002-1H-stable-1-3ed"
  TAG="official-v002-1h-stable-1-3ed"
  STAMP="$(date +%Y%m%d-%H%M%S)"
  BRANCH="update/v002-1h-stable-1-3ed-return-status-${STAMP}"
  BACKUP="/workspaces/For-ecalendar-v2-backup-before-1-3ed-${STAMP}.tar.gz"
  TEMP_ZIP="/workspaces/.for-e-1-3ed-package-${STAMP}.zip"

  cd "$PROJECT"

  echo "======================================"
  echo "1. 尋找並驗證 1-3ed 完整專案 ZIP"
  echo "======================================"
  ...
  ls -lh "$ZIP_PATH"
  unzip -tq "$ZIP_PATH"
  unzip -p "$ZIP_PATH" src/main.js | grep -F "const APP_VERSION = 'V002-1H-stable-1-3ed'"
  unzip -p "$ZIP_PATH" src/main.js | grep -F "const OFFICIAL_VERSION = 'official-v002-1h-stable-1-3ed'"
  unzip -p "$ZIP_PATH" src/main.js | grep -F "const RETURN_HOME_STATUS_LOGIC_VERSION = '1-3ed'"
  ...
  if ! printf '%s' "$REMOTE_VERSION" | grep -Eq "1-3ec|1-3ed"; then
    echo "遠端 main 不是預期的 1-3ec / 1-3ed，為避免覆蓋其他更新，本次部署停止。"
    false
  fi

  echo
  echo "======================================"
  echo "5. 從最新 main 建立 1-3ed 更新分支"
  echo "======================================"
  ...
  echo "8. 確認返鄉狀況嚴格判斷修正"
  echo "======================================"
  grep -nF "const APP_VERSION = 'V002-1H-stable-1-3ed'" src/main.js
  grep -nF "const OFFICIAL_VERSION = 'official-v002-1h-stable-1-3ed'" src/main.js
  grep -nF "const RETURN_HOME_STATUS_LOGIC_VERSION = '1-3ed'" src/main.js
  grep -nF "function isPersonnelAffairsReturnHomeSchedule" src/main.js
  grep -nF "if (!isCoreLeaveMeetingCategory(row.category)) return false" src/main.js
  grep -nF "return getCoreLeaveMeetingSubtypeFromRow(row) === '返鄉'" src/main.js
  grep -nF "if (key === '返鄉') return isPersonnelAffairsReturnHomeSchedule(row)" src/main.js
  grep -nF "return isPersonnelAffairsReturnHomeSchedule(row)" src/main.js
  ...
  if git diff --cached --quiet; then
    echo "目前 main 已包含相同的 1-3ed 程式，沒有新差異需要提交。"
  else
    git commit -m "fix: restrict return-home status to personnel affairs subtype (1-3ed)"
  fi
