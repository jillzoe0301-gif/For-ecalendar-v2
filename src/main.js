...
function getManagedAdministrativeReminderItems() {
  const configured = getManagedListOption('administrativeReminderItems', administrativeReminderItems)
  return ['--', ...configured]
...
function renderApp() {
  ...
  const overviewMonthToolbarInput = document.querySelector('#overviewMonthToolbarInput')
  ...
  const fieldMonthToolbarInput = document.querySelector('#fieldMonthToolbarInput')
  ...
  const meetingCalendarViewModeSelect = document.querySelector('#meetingCalendarViewModeSelect')
  ...
  const meetingMonthToolbarInput = document.querySelector('#meetingMonthToolbarInput')
  ...
  document.querySelectorAll('[data-line-clear-selection-inside]').forEach(button => {
  ...
  document.querySelectorAll('[data-line-preview-copy-single]').forEach(button => {
  ...
  if (colorSettingsForm) {
    colorSettingsForm.addEventListener('submit', async event => {
      ...
      getScheduleColorDefinitions().forEach(item => {
        ...
        if (manualValue && !normalizedManual) invalidLabels.push(item.label)
        nextColors[item.key] = normalizedManual || normalizedPicker || item.defaultColor
      })

      if (invalidLabels.length) {
        alert(`以下色碼格式不正確，請輸入 # 加 6 碼，例如 #FAAB78：\n${invalidLabels.join('、')}`)
        return
      }
  ...
  if (addBtn) {
    addBtn.addEventListener('click', () => {
      if (currentPage === 'fieldSchedule' || currentPage === 'fieldDetail') {
        if (!canCreateFieldSchedule()) return denyPermission('你的角色沒有新增外務行程權限。')
        openFieldScheduleModal()
        return
      }
      if (!canCreateForCurrentPage()) return denyPermission()
      if (currentPage === 'meetingRoom') openMeetingRoomModal()
  ...
  document.querySelectorAll('[data-complete-continuous]').forEach(btn => {
    btn.addEventListener('click', async event => {
      event.preventDefault()
      event.stopPropagation()
      await completeSchedule(btn.dataset.completeContinuous)
    })
  })
...
function renderFieldScheduleCalendar() {
  ...
  return `
    ...
    ${renderFieldCalendarBody(staffRows, weekDates, todayKey)}
...
function getFieldDetailStaffOptionsHtml() {
  ...
  return `<option value="全部" ${fieldDetailFilters.staffId === '全部' ? 'selected' : ''}>全部人員</option>` +
    rows.map(staff => `
      <option value="${staff.staff_id}" ${fieldDetailFilters.staffId === staff.staff_id ? 'selected' : ''}>${escapeHtml(staff.name || '-')}</option>
    `).join('')
}
...
function isVehicleMaintenanceSchedule(row = {}) {
  return String(row?.category || '') === '公務車保養' || String(row?.schedule_type || '') === '公務車保養'
}
...
function getDepartmentSupervisorStaffIds(departmentId = '', departmentName = '') {
  const normalizedDepartmentName = String(departmentName || '').trim()
  const rows = staffList.filter(staff => {
    if (!staff?.staff_id || staff.deleted_at) return false
    if ((staff.status || '啟用') !== '啟用') return false

    const sameDepartment = departmentId
      ? staff.department_id === departmentId
      : (normalizedDepartmentName ? staff.department_name === normalizedDepartmentName : true)

    if (!sameDepartment) return false

    const roleText = [staff.role, staff.position, staff.position_name, staff.title]
      .filter(Boolean)
      .join('｜')
    return roleText.includes('主管')
  })

  if (rows.length) return rows.map(staff => staff.staff_id)

  return staffList
    .filter(staff => staff?.staff_id && !staff.deleted_at && (staff.status || '啟用') === '啟用')
    .filter(staff => [staff.role, staff.position, staff.position_name, staff.title].filter(Boolean).join('｜').includes('主管'))
...
function renderMeetingRoomCalendar() {
  ...
  const todayKey = todayString()
  const roomRows = getManagedListOption('meetingRoomOptions', meetingRoomOptions)
  ...
  return `
    ...
    ${renderMeetingCalendarBody(roomRows, weekDates, todayKey)}
  `
}
  ...
  modal.innerHTML = `
    <div class="modal-panel">
      ...
      <form id="meetingRoomForm" class="form-grid">
        ...
        <label class="span-2 edit-common-description-field">
          內容 / 備註
  ...
  return `
    <section class="line-select-card line-select-dropdown-card">
      ...
      <details class="line-select-dropdown ${noneSelected ? 'is-none-selected' : ''}">
        ...
        <div class="line-select-dropdown-tools">
          <button type="button" class="secondary-btn line-clear-selection-btn" data-line-clear-selection-inside>一鍵取消勾選</button>
          <span>${noneSelected ? '目前 0 筆通知' : (useAll ? `目前全部 ${rows.length} 筆` : `目前已選 ${selected.size} 筆`)}</span>
        </div>
...
function buildSingleLineNotifyMessage(row) {
  if (!row) return ''
  const today = todayString()
  const targetText = getLineNotifyTargetText()
  return [
    `FOR-e｜${lineNotifyState.type}通知`,
    `${today}｜對象：${targetText}`,
    '',
    formatLineScheduleItem(row)
  ].join('\n')
}

function renderLineNotifyPreviewList(rows = [], totalCount = 0) {
  return `
    <section class="line-preview-list line-preview-single-copy-list">
      <div class="section-title-row line-preview-title-row">
        <h4>本次通知資料預覽</h4>
        <span>${rows.length} / ${totalCount} 筆</span>
      </div>
      ${rows.length ? rows.map(row => `
        <article class="line-preview-item-card">
      ...
      `).join('') : '<div class="empty-state">目前沒有符合條件的通知資料。</div>'}
    </section>
  `
}
...
function renderLineNotificationPage() {
  ...
  return `
    ...
    ${renderLineNotifyPreviewList(selectedRows, rows.length)}
  `
}
  ...
  modal.innerHTML = `
    <div class="modal-panel login-reminder-modal">
      ...
      <div class="login-reminder-summary">
        <div><strong>${(groups.birthdayRows?.length || 0) + (groups.personalBirthday ? 1 : 0)}</strong><span>生日</span></div>
        ...
        <div><strong>${(groups.birthdayRows?.length || 0) + (groups.personalBirthday ? 1 : 0) + groups.todayTodos.length + groups.todaySchedules.length + groups.reminderRows.length}</strong><span>合計</span></div>
      </div>

      <div class="login-reminder-body">
        ${renderLoginPersonalBirthdayGreeting(groups.personalBirthday)}
...
function getDatesInCurrentMonth() {
  return getDatesByMonthValue(getOverviewActiveMonth())
}
...
function getOverviewCalendarLabel(weekDates = [], viewMode = getOverviewViewMode()) {
  ...
  if (viewMode === '個人當月') return `${getOverviewActiveMonth()}｜個人當月`
  if (viewMode === '月份顯示') return `${getOverviewActiveMonth()}｜月份顯示`
...
function normalizeMonthValue(value = '') {
  const text = String(value || '').trim()
  return /^\d{4}-\d{2}$/.test(text) ? text : getCurrentMonthValue()
}

function shiftMonthValue(value = '', delta = 0) {
...
function getOverviewActiveMonth() {
  return normalizeMonthValue(overviewDisplayMonth || getCurrentMonthValue())
}

function getDatesByMonthValue(value = '') {
  const normalized = normalizeMonthValue(value)
  const [year, month] = normalized.split('-').map(Number)
...
function renderOverviewMonthSlidingTable(staffRows = [], monthDates = [], todayKey = todayString(), tableClass = 'is-overview-month') {
  if (!staffRows.length) return '<div class="empty-state">目前沒有可顯示的人員。</div>'

  return `
    <div class="week-overview-scroll month-sliding-scroll">
...
function renderToolbarMonthInput(id, monthValue = getCurrentMonthValue(), label = '顯示月份') {
  return `
    <label class="toolbar-month-control">
      <span>${label}</span>
      <input id="${id}" type="month" value="${escapeHtml(normalizeMonthValue(monthValue))}">
    </label>
  `
}

function renderToolbarViewModeSelect(id, value = '週檢視') {
  return `
    <label class="toolbar-month-control toolbar-view-control">
      <span>檢視</span>
      <select id="${id}">
        ${getCalendarViewModeOptionsHtml(value)}
      </select>
    </label>
  `
}

function getFieldActiveMonth() {
  return normalizeMonthValue(fieldDisplayMonth || getCurrentMonthValue())
}

function getMeetingActiveMonth() {
  return normalizeMonthValue(meetingDisplayMonth || getCurrentMonthValue())
}

function getFieldCalendarDates() {
  if (fieldCalendarViewMode === '月份顯示') return getDatesByMonthValue(getFieldActiveMonth())
  return getWeekDates(fieldWeekOffset)
}

function getMeetingCalendarDates() {
  if (meetingCalendarViewMode === '月份顯示') return getDatesByMonthValue(getMeetingActiveMonth())
  return getWeekDates(meetingWeekOffset)
}

function getFieldCalendarLabel(dates = getFieldCalendarDates()) {
  if (!dates.length) return ''
  if (fieldCalendarViewMode === '月份顯示') return `${getFieldActiveMonth()}｜月份顯示`
  return getWeekLabel(dates)
}

function getMeetingCalendarLabel(dates = getMeetingCalendarDates()) {
  if (!dates.length) return ''
  if (meetingCalendarViewMode === '月份顯示') return `${getMeetingActiveMonth()}｜月份顯示`
  return getWeekLabel(dates)
}

function renderFieldSingleMonthCalendar(staff, monthDates = [], todayKey = todayString()) {
  if (!staff || !monthDates.length) return '<div class="empty-state">目前沒有可顯示的外務人員。</div>'
...
function renderFieldMonthSlidingTable(staffRows = [], monthDates = [], todayKey = todayString()) {
  if (!staffRows.length) return '<div class="empty-state">目前沒有可顯示的外務人員。</div>'

  return `
    <div class="field-week-scroll month-sliding-scroll">
      <table class="field-week-table is-field-month">
        <thead>
          <tr>
...
function renderMeetingMonthTable(roomRows = [], monthDates = [], todayKey = todayString()) {
  return `
    <div class="meeting-week-scroll month-sliding-scroll">
      <table class="meeting-week-table is-meeting-month">
        <thead>
          <tr>
...
function renderMeetingCalendarBody(roomRows = [], dates = [], todayKey = todayString()) {
  if (meetingCalendarViewMode === '月份顯示') return renderMeetingMonthTable(roomRows, dates, todayKey)
...
function getCurrentBirthdayStaffRow() {
  return staffList.find(staff => staff.staff_id === currentProfile?.staff_id)
    || allStaffList.find(staff => staff.staff_id === currentProfile?.staff_id)
    || currentProfile
    || {}
}

function isCurrentUserBirthdayToday() {
  return isStaffBirthdayOnDate(getCurrentBirthdayStaffRow(), todayString())
}

function renderLoginPersonalBirthdayGreeting(isBirthday = false) {
...
function getOverviewQuickGroupStaffPool() {
  const rows = [
    ...(typeof getOverviewBaseStaffRows === 'function' ? getOverviewBaseStaffRows() : []),
    ...(Array.isArray(staffList) ? staffList : []),
    ...(Array.isArray(allStaffList) ? allStaffList : []),
    ...(typeof getUserManageRows === 'function' ? getUserManageRows() : [])
  ]

  const map = new Map()
  rows.forEach(row => {
    const staffId = normalizeStaffId(row?.staff_id || row?.staffId || row?.id || '')
    if (!staffId) return
    if (row?.deleted_at || row?.deletedAt || row?.is_deleted === true) return
    const current = map.get(staffId) || {}
    map.set(staffId, { ...current, ...row, staff_id: staffId })
  })

  return sortStaffRowsByFilter([...map.values()], { sortBy: 'display_order', sortDir: 'asc' })
}

function getOverviewQuickGroupStaffIdsByNames(names = []) {
  const wanted = new Set((names || []).map(normalizeOverviewQuickGroupNameText).filter(Boolean))
  if (!wanted.size) return []
  return getOverviewQuickGroupStaffPool()
    .filter(staff => {
      const name = normalizeOverviewQuickGroupNameText(staff.name || staff.staff_name || staff.full_name || staff.display_name || '')
      return wanted.has(name)
    })
    .map(staff => normalizeStaffId(staff.staff_id))
    .filter(Boolean)
}
...
function getAdministrativeQuickGroupStaffIds() {
  return getOverviewQuickGroupStaffPool()
    .filter(staff => {
      ...
      return name === '徐嘉陽' || positionText.includes('行政') || positionText.includes('海外行政') || departmentText.includes('營運處')
    })
    .map(staff => normalizeStaffId(staff.staff_id))
    .filter(Boolean)
}
  ...
  return `
    <div class="page-toolbar">
      ...
      <div class="toolbar-actions overview-toolbar-actions">
        ...
        ${showWeekNav ? `
          ...
          <button class="secondary-btn" id="nextWeekBtn">${isMonthMode ? '下一月' : '下一週'}</button>
        ` : ''}
    ...
    <form id="overviewFilterForm" class="overview-filter-panel overview-filter-panel-compact">
      <div class="overview-filter-compact-row">
        ...
        <details class="compact-multi-select compact-filter-control ${viewMode.startsWith('個人') ? 'is-disabled-filter' : ''}">
...
function isExternalTrainingScheduleForFieldHint(row = {}) {
  ...
  const colorKey = typeof getScheduleColorKey === 'function' ? getScheduleColorKey(row) : ''
  if (colorKey === '請假' || colorKey === '返鄉') return false
  ...
  // 不要把大分類「請假 / 會議 / 活動 / 外訓」中的「外訓」當作外訓提示來源；
  // 只看實際細項、標題、內容，避免同一筆請假 / 休假同時顯示「訓」與「休」。
  if (leaveReturnPattern.test(contentText) && !trainingPattern.test(contentText)) return false

  if (colorKey === '外訓' && trainingPattern.test(contentText)) return true
  if (row.category === '請假 / 會議 / 活動 / 外訓' && trainingPattern.test(contentText)) return true
  if (trainingPattern.test(normalizedText)) return true
  ...
  modal.innerHTML = `
    <div class="modal-panel">
      ...
      <form id="fieldScheduleForm" class="form-grid">
        <div class="span-2 field-staff-dropdown-box">
          <div class="field-title">外務人員</div>
          <details class="field-staff-dropdown">
            <summary>選擇外務人員</summary>
            <div class="field-staff-dropdown-panel">
              ${fieldStaffOptionsHtml(defaultStaffId) || '<div class="empty-state">目前沒有可選外務人員。</div>'}
            </div>
          </details>
        </div>
...
function cleanServiceAdminNotifyNote(noteText = '') {
  return String(noteText || '')
    .split('｜')
    .map(item => item.trim())
    .filter(Boolean)
...
function hasExistingServiceAdministrativeTodo(sourceScheduleId = '', adminStaffId = '') {
  const sourceId = String(sourceScheduleId || '').trim()
  const staffId = String(adminStaffId || '').trim()
  if (!sourceId || !staffId) return false

  return (schedules || []).some(row => {
    if (!row || row.status === '取消') return false
    if (row.category !== '待辦事項') return false
    const note = String(row.sub_type_note || '')
    if (!note.includes(`來源行程：${sourceId}`)) return false
    if (!note.includes('服務行程通知行政')) return false
    return getActiveAssigneeIds(row).includes(staffId)
  })
}

async function maybeCreateServiceAdministrativeTodo(sourceRow = {}, scheduleType = '', taskDetail = '', adminStaffId = '', options = {}) {
  const staffId = String(adminStaffId || '').trim()
  if (!staffId) return null

  if (sourceRow.schedule_id && hasExistingServiceAdministrativeTodo(sourceRow.schedule_id, staffId)) return null

  const typeText = String(scheduleType || sourceRow.schedule_type || '服務行程通知').trim() || '服務行程通知'
  return createAdministrativeTodoSchedule(sourceRow, typeText, taskDetail || sourceRow.description || sourceRow.title || '', staffId, {
    sourceLabel: '服務行程通知行政',
    startDate: options.startDate || sourceRow.start_date || todayString(),
    extraNote: options.extraNote || '由服務行程「通知行政」欄位建立。'
  })
}
  ...
  modal.innerHTML = `
    <div class="modal-panel">
      ...
      <form id="editScheduleForm" class="form-grid" data-current-category="${escapeHtml(row.category || '')}">
        ...
        <div class="span-2 form-section hidden vehicle-maintenance-section" id="editVehicleMaintenanceBlock">
          <label>
            公務車選擇
            <select name="maintenance_car">
              ${maintenanceCarOptions}
            </select>
          </label>

          <label>
            保養日期
            <input name="maintenance_start_date" type="date" value="${escapeHtml(maintenanceStartDate)}">
          </label>

          <label>
            歸還日期
            <input name="maintenance_return_date" type="date" value="${escapeHtml(maintenanceReturnDate)}">
          </label>
        ...
        <div class="span-2 form-section hidden" id="editLeaveMeetingBlock">
          ...
          <label>
            代理人
            <select name="edit_proxy_staff_id">
              ${staffSelectOptionsHtmlSelected(editProxyStaffId)}
            </select>
          </label>
        </div>
        ...
        <label class="span-2 edit-common-title-field">
          標題
        ...
        <label class="edit-time-type-field">
          時間類型
        ...
        <label class="span-2 edit-note-field">
          備註 / 提醒 / 證件
...
async function saveEditedSchedule(event, modal, originalRow) {
  ...
  let payloadTitle = form.get('title')
  let payloadDescription = form.get('description') || null
  let payloadStartDate = form.get('start_date')
  let payloadEndDate = getScheduleModeEndDate(form)
  let payloadTimeType = form.get('time_type')
  ...
  if (category === '公務車保養') {
    ...
    editSubType = maintenanceCar || '公務車保養'
    payloadTitle = `公務車保養｜${maintenanceCar || '未指定車輛'}`
    payloadDescription = [
      `公務車：${maintenanceCar || '-'}`,
    ...
    payloadEndDate = maintenanceReturnDate
    payloadTimeType = '不指定'
    payloadStartTime = null
    payloadEndTime = null
    payloadCustomerName = maintenanceCar || null
    payloadLocationName = '公務車保養'
    payloadAddress = null
    payloadCarNo = maintenanceCar || null
    editSubTypeNote = [
      buildRepeatNote({ get: name => {
  ...
  if (category === '請假 / 會議 / 活動 / 外訓') {
    ...
    editSubTypeNote = appendNotifySupervisorNote([
      ...
      editProxyName ? `代理人：${editProxyName}` : ''
...
/* FOR-e V002-1P-78 START - options 2-3 columns template 10cm */
/*
  V002-1P-78｜選項管理二至三欄與行程內容 10cm 欄位
  - 一般選項管理改成一排最多三個區塊，寬度不足時自動變兩個或一個
  - 行程類型對應內容改成一排兩個區塊
  - 行程類型欄位靠左對齊
...
/* FOR-e V002-1P-135 START - selective fixes from v110 */
/*
  V002-1P-135｜V110 後精選功能修正
  - 篩選人員名稱不顯示部門
  - 個人當月行事曆、多人月份橫向檢視、月份 / 年份控制
  - 今日生日提醒
  - 外務端午淡橘背景、今日外務 #FAAB78
  - 請假 / 會議 / 活動 / 外訓拆分調色，色碼可手動輸入
  - 會議室卡片不顯示與會人員，查看行程保留與會資訊
  - 外務明細新增外務權限修正
  - 新增外務人員改為下拉選擇
*/
/* FOR-e V002-1P-135 END - selective fixes from v110 */

/* FOR-e V002-1P-136 START - month controls outside filters and 4px borders */
/*
  V002-1P-136｜月份控制移出篩選列、外框 4px 與請假色修正
  - 取消 V135 行程總覽篩選列內的月份欄，恢復原本篩選樣式
  - 行程總覽、外務行程、會議室改在工具列提供檢視 / 月份控制
  - 多人月份橫向拉動，單人月份行事曆格狀
  - 請假 / 休假等外框套用獨立顏色
  - 行程表卡片外框加粗為 4px
  - 外務行程當日外框色改為 #FAAB78
...
/* FOR-e V002-1P-168 START - cleanup old override blocks */
/*
  V002-1P-168｜清理舊覆蓋區塊
  - 移除近期已被後續版本覆蓋的說明區塊
  - 不刪除實際功能函式，只清除舊版殘留標記
  - 更新檔內只保留 src/main.js、src/style.css、public/icons/cake.png
*/
...
// FOR-E v002-1p-280-2 START: precise follow type assigned filter
(() => {
  const VERSION_FLAG = '__FOR_E_V002_1P_280_2_PRECISE__'
  ...
  function findFollowTypeBlocks() {
    const titleCandidates = Array.from(document.querySelectorAll('label, legend, h1, h2, h3, h4, div, span, p, strong'))
      .filter((element) => {
        const text = textOf(element)
        return text === '下一次追蹤類型' || text.startsWith('下一次追蹤類型')
      })

    const blocks = []
    titleCandidates.forEach((title) => {
      let panel = title.parentElement
      for (let i = 0; i < 8 && panel; i += 1, panel = panel.parentElement) {
        const panelText = textOf(panel)
        const radioCount = panel.querySelectorAll('input[type="radio"]').length
        if (panelText.includes('追蹤') && panelText.includes('單純紀錄') && radioCount >= 2) {
          blocks.push(panel)
          break
        }
      }
    ...
    if (blocks.length) return uniqueElements(blocks)

    return Array.from(document.querySelectorAll('section, fieldset, .form-section, .form-card, .modal-body, .modal-content, div'))
      .filter((element) => {
        const text = textOf(element)
        return text.includes('下一次追蹤類型') && text.includes('追蹤') && text.includes('單純紀錄') && element.querySelectorAll('input[type="radio"]').length >= 2
      })
      .slice(0, 8)
  }

  function optionElementForRadio(input, panel) {
    let option = input.closest('label')
    if (option && option !== panel && panel.contains(option)) return option

    option = input.parentElement
    for (let i = 0; i < 5 && option && option !== panel; i += 1) {
      const optionText = textOf(option)
      if (optionText.includes('單純紀錄') || optionText.includes('追蹤')) return option
  ...
  function enhanceFollowTypeLayout() {
    const panels = findFollowTypeBlocks()
    panels.forEach((panel) => {
      if (!isVisible(panel)) return
      const radios = Array.from(panel.querySelectorAll('input[type="radio"]'))
        .filter((input) => {
          const optionText = textOf(optionElementForRadio(input, panel))
          return optionText.includes('追蹤') || optionText.includes('單純紀錄')
        })
        .slice(0, 2)

      if (radios.length < 2) return

      const options = uniqueElements(radios.map((input) => optionElementForRadio(input, panel)))
      if (options.length < 2) return

      panel.classList.add('for-e-follow-type-inline-panel')

      let row = panel.querySelector(':scope > .for-e-follow-type-options-row')
      if (!row) {
        row = document.createElement('div')
        row.className = 'for-e-follow-type-options-row'
        options[0].parentElement.insertBefore(row, options[0])
      }

      options.forEach((option) => {
        option.classList.add('for-e-follow-type-option')
        const input = option.querySelector('input[type="radio"]')
        if (input) input.classList.add('for-e-follow-type-input')
        if (option.parentElement !== row) row.appendChild(option)
      })
    })
  }

  function findAssignedPageTitle() {
    return Array.from(document.querySelectorAll('h1, h2, h3, h4, .page-title, .section-title, div, span'))
      .find((element) => textOf(element).includes('我指派的事項追蹤'))
  }

  function findAssignedRoot() {
    const title = findAssignedPageTitle()
    if (!title) return null
    let root = title.closest('main, section, .page, .panel, .content, .view, .card') || title.parentElement
    for (let i = 0; i < 5 && root?.parentElement; i += 1) {
      const text = textOf(root)
      if (text.includes('我指派的事項追蹤') && text.length > 80) return root
      root = root.parentElement
    }
    return title.closest('main') || title.parentElement
  }

  function isProbablyScheduleCard(element) {
    if (!element || element.id === 'for-e-assigned-completed-filter-bar') return false
    if (element.closest('#for-e-assigned-completed-filter-bar')) return false
    const text = textOf(element)
    if (text.length < 10) return false
    if (text.includes('完成狀態') || text.includes('隱藏已完成') || text.includes('只看已完成') || text.includes('顯示全部')) return false
    const className = String(element.className || '')
    const classLooksLikeCard = /(card|item|row|task|schedule|list|record|assignment)/i.test(className)
    const textLooksLikeCard = /(已完成|未完成|已結束|查看|取消|指派|執行者|建立者|時間|日期)/.test(text)
    return classLooksLikeCard && textLooksLikeCard
  }

  function findAssignedCards(root) {
    if (!root) return []
    const selectors = [
      '.schedule-card', '.task-card', '.item-card', '.assignment-card', '.record-card', '.list-card', '.card',
      '[class*="card"]', '[class*="item"]', '[class*="task"]', '[class*="schedule"]', '[class*="record"]'
    ]
    return uniqueElements(selectors.flatMap((selector) => Array.from(root.querySelectorAll(selector))))
      .filter(isProbablyScheduleCard)
  }

  function isCompletedCard(card) {
    const text = textOf(card)
    if (text.includes('未完成')) return false
    return text.includes('已完成') || text.includes('已結束') || text.includes('完成時間')
  }

  function applyAssignedCompletedFilter() {
    const root = findAssignedRoot()
    const select = document.getElementById('for-e-assigned-completed-filter-select')
    if (!root || !select) return

    const mode = select.value || 'hide-completed'
    findAssignedCards(root).forEach((card) => {
      const completed = isCompletedCard(card)
      const shouldHide = (mode === 'hide-completed' && completed) || (mode === 'only-completed' && !completed)
      card.classList.toggle('for-e-completed-filter-hidden', shouldHide)
    })
  }

  function installAssignedCompletedFilter() {
    const title = findAssignedPageTitle()
    const root = findAssignedRoot()
    if (!title || !root) return
    if (document.getElementById('for-e-assigned-completed-filter-bar')) {
      applyAssignedCompletedFilter()
      return
    }

    const bar = document.createElement('div')
    bar.id = 'for-e-assigned-completed-filter-bar'
    bar.className = 'for-e-assigned-completed-filter-bar'
    bar.innerHTML = `
      <label class="for-e-assigned-completed-filter-label" for="for-e-assigned-completed-filter-select">完成狀態</label>
      <select id="for-e-assigned-completed-filter-select" class="for-e-assigned-completed-filter-select">
        <option value="hide-completed" selected>隱藏已完成</option>
        <option value="only-completed">只看已完成</option>
        <option value="all">顯示全部</option>
      </select>
    `

    const titleRow = title.closest('.toolbar, .page-header, .section-header, .card-header, .header') || title.parentElement
    if (titleRow) {
      titleRow.classList.add('for-e-assigned-filter-host')
      titleRow.appendChild(bar)
    } else {
      root.insertBefore(bar, root.firstChild)
    }

    bar.querySelector('select')?.addEventListener('change', applyAssignedCompletedFilter)
    applyAssignedCompletedFilter()
  }

  function runForE280Enhancements() {
    enhanceFollowTypeLayout()
    installAssignedCompletedFilter()
    applyAssignedCompletedFilter()
  }
  ...
  function scheduleRun() {
    ...
    window.requestAnimationFrame(() => {
      scheduled = false
      runForE280Enhancements()
    })
  }

  document.addEventListener('change', (event) => {
    if (event.target?.id === 'for-e-assigned-completed-filter-select') applyAssignedCompletedFilter()
  })

  document.addEventListener('DOMContentLoaded', scheduleRun)
  window.addEventListener('hashchange', () => setTimeout(scheduleRun, 80))
  window.addEventListener('popstate', () => setTimeout(scheduleRun, 80))
  setInterval(scheduleRun, 1200)
...
// FOR-E v002-1p-280-2 END: precise follow type assigned filter
