...
const newFieldScheduleRow = `const FIELD_PUBLIC_DUTY_DISPLAY_LOGIC_VERSION = '1-3en'
...
const newRenderFieldScheduleCard = `function renderFieldScheduleCard(row) {
  if (typeof isFieldDayReminderSchedule === 'function' && isFieldDayReminderSchedule(row)) return ''
  // 1-3en：公差外出只借用外務頁顯示資格，卡片維持原本公差外出樣式與標題。
...
const newSimpleFieldCardDecision = `function shouldUseSimpleFieldScheduleCard(row = {}) {
  if (!row) return false
  // 1-3en：公差外出雖可出現在外務頁，但不可轉成外務卡片樣式。
...
const newFieldColorDecision = `  // 1-3en：公差外出保留原本公差外出顏色，不因出現在外務頁而變成外務橘色。
...
const forEPhase4CompatibilityPatch = {
  name: 'for-e-1-3en-field-public-duty-original-card-style',
  enforce: 'pre',
  transform(code, id) {
    if (!id.replaceAll('\\', '/').endsWith('/src/main.js')) return null

    let next = code
      .replace(/const APP_VERSION = 'V002-1H-stable-1-3e[a-z]'/, "const APP_VERSION = 'V002-1H-stable-1-3en'")
      .replace(/const OFFICIAL_VERSION = 'official-v002-1h-stable-1-3e[a-z]'/, "const OFFICIAL_VERSION = 'official-v002-1h-stable-1-3en'")
    ...
    next = next
      .replace("const FIELD_PUBLIC_DUTY_DISPLAY_LOGIC_VERSION = '1-3ek'", "const FIELD_PUBLIC_DUTY_DISPLAY_LOGIC_VERSION = '1-3en'")
      .replace("const FIELD_PUBLIC_DUTY_DISPLAY_LOGIC_VERSION = '1-3el'", "const FIELD_PUBLIC_DUTY_DISPLAY_LOGIC_VERSION = '1-3en'")
      .replace("const FIELD_PUBLIC_DUTY_DISPLAY_LOGIC_VERSION = '1-3em'", "const FIELD_PUBLIC_DUTY_DISPLAY_LOGIC_VERSION = '1-3en'")
